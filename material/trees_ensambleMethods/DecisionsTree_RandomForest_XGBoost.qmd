...
path <- '/Users/ramitjans/Downloads/idealista18_BCN_conRenta.csv'
BCN <- read.csv2(path)
```
...
pisos$rtrain <- 
  ...
  select(-RENTA) %>% 
  as.matrix() %>% 
  xgb.DMatrix(data = ., label = rtrain$RENTA)
...
# Todos son parámetros por defecto exepto eta
tuneGrid <- expand.grid(nrounds = 5,  # Menor cantidad de rondas 
                       max_depth = 6,   # Profundidad más baja para los árboles
...
## Si no decimos nada en type (type = prob), nos devolvera la probabilidad de 
## pertenecer a cada clase. 
prediccion <- predict(model$finalModel, rtest, type = "class")
